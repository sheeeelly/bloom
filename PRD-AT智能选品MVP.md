...
> 版本：**v0.5.2** · 日期：2026-07-24 · 品类试点：**AT（Atelier / 高定线）**  
> 受众：产品 / 运营 / 研发 / 数据 / 供应链  
> 依据：产品 Owner 审阅修订（§1–3）+ grill/Act2 工程约束择要落地  
> 数据口径参考：  
> - [产品视角_竞品监控雷达项目设计_侧方案.md](./产品视角_竞品监控雷达项目设计_侧方案.md)  
> - [上新下架表逻辑说明.md](./上新下架表逻辑说明.md)  
> - **选品提案 AI 参考**：[amazon-product-manager-skill](https://github.com/KKKKkrisPhillllll/amazon-product-manager-skill/)（决策框架见 §0.A）  
>
> **权威顺序（v0.5）**  
> 1. 本 PRD 中 **产品目标 / KR / 用户流 / 功能边界**（Owner 已改）  
> 2. 本 PRD **F1+ 实现合同**（字段、状态机、API）  
> 3. [PLAN.md](./PLAN.md) 中与上文不冲突的工程加固项  
> 若冲突：**以本 PRD v0.5 为准**，并回写 PLAN。  
>
> **v0.5.2**：突出参考 amazon-product-manager-skill 决策框架（生成选品提案）。  
> **v0.5.1**：补充 §2.3 用户矩阵。  
> **v0.5**：对齐 Owner；F1+ 可编码；库存可生产性；埋点不干扰审批。  
> **v0.4**：Act2 工程加固择要并入。
...
埋点：方案/提案系统字段标记来源，不进入审批人判断文案
AI建议写法：对齐 amazon-product-manager-skill 决策骨架（§0.A）
```
...
| 定价 | 基于 BOM 评价格优势 | F5 |
| 供应链 | **基于库存信息**评可生产性 | F6 |
| AI | 正式建议环节 | F7 + **§0.A 参考 Skill** |
| 埋点 | 能归因即可，勿干扰审核观感 | F9 |

---

## 0.A 参考 Skill：选品提案决策框架（突出）

> 参考：[amazon-product-manager-skill](https://github.com/KKKKkrisPhillllll/amazon-product-manager-skill/)  
> 用途：指导 **如何把证据收成可审的选品提案建议**（Product Selection Memo 形态），非 Amazon 爬虫、非自动出爆品。

### 借鉴的重要决策纪律

1. **结论先行**：先 Decision，再证据。  
2. **枚举决策**：`Go / Test First·Rework / No-Go` → 本方案 `建议推 / 谨慎·MTO / 不建议推`。  
3. **多维再判**：需求·竞争·利润·差异化·运营·资金 → 本地化为上新热销、内部同款、BOM 价带、舆情、库存可生产性。  
...
机会与草稿（F1–F4）→ 规则算成本/库存（F5–F6）→ **按上述骨架生成 AI 建议（F7）** → 审批写入（F8–F9）。  
本期不照搬 FBA/ACoS/关键词市场筛选层；雷达上新池已承担「找市场切口」。

---

## 1. Summary

本项目为 Azazie **AT（Atelier / 高定礼服线）** 品类建设智能化选品闭环：聚合竞品分析、社媒舆情、AZ 相似款历史销量表现，生成可编辑选品方案；基于 **BOM 标准成本 ×5** 与竞品价带判断价格竞争力；经 **AI 建议** 与 **品类负责人审批** 后写入选款提案。目标是缩短信息搜集与方案准备时间，提高可上新质量，并通过轻量埋点为网站 **增量 GMV** 归因打底。
...
智能选品平台 Demo 已具备机会池 / 监控 / 提案，但缺少 **可量化的选品规则、成本预估、AI 建议与审批状态机**。

### 2.2 What changed vs 一期方案
...
| 角色 | 主要页面 | 关键动作 | MVP 范围 |
|------|----------|----------|----------|
| **品类运营**（AT） | 监控报告 · 机会池 · 方案草稿/编辑 · 社媒证据 | 浏览上新×流行机会 → 录入社媒 → **生成方案** → 跑成本/可生产性 → 提交 AI 建议 → 提交审批；（审批通过后由系统）**同步选款** | **本期主用户** |
| **品类负责人**（AT） | 审批队列 · 方案详情（证据区） | 查看 AI 建议 / gates / 成本价带 / 库存结论 → **通过或驳回**（驳回必填原因）；高亮风险与冲突点 | **本期审批权威** |
| **商品负责人** | 方案可生产性与成本只读视图；（Later）内部引擎四象限 | 本期：查阅库存/BOM 结论作协同参考；Later：扩产规划、象限策略 | **本期只读协同**；扩产规划后置 |
| **管理层** | 总览 / 监控报告摘要 | 查看周度 **Top 机会**与避雷（下架/高风险摘要）；不参与单方案审批 | **本期轻量只读**；看板深化后置 |

**权限原则（实现）**

- 品类运营：本品类 AT 的机会与方案 CRUD；不可终审通过。  
- 品类负责人：本品类审批通过/驳回；可看全量证据，审批页不展示 `generation_source` 推荐文案。  
- 商品负责人 / 管理层：默认只读；无写入选款权限。  
- 角色与页面映射用于路由守卫与菜单裁剪（M1 起按上表配置）。
...
让商品运营在一个工作台完成「看见机会 → 写清改款与定价 → AI 建议 → 负责人审批 → 进选款」，减少无效跟款（价无优势、雷款、做不了），提升上新质量与速度，服务网站增量。

### 3.2 Key Results（SMART）

| KR | 指标 | 一期目标 | 测量 |
|----|------|----------|------|
| KR1 | 竞品上新覆盖 + 选品周期 | **不漏**监控站本轮「上新且列表靠前」机会进入机会池可操作；选品节奏从月/周级缩短为 **按爬取批次可跟**（默认跟雷达周频；日频爬取成本另立项） | 机会池覆盖率；方案创建时间戳 |
| KR2 | 单方案「信息搜集+成稿」时长 | **−50%** | 抽样日记 / 系统时间戳 |
| KR3 | AI 建议覆盖率 | **100%** 进入负责人审批的方案均有 AI 建议记录 | 状态机 |
| KR4 | 负责人一次通过率 | ≥ **80%**（驳回必填原因，原因回流优化提示词/规则） | 审批日志 |
| KR5 | 增量埋点完备 | 100% 写入选款的提案带系统字段 `generation_source=system_hub_at`；**审批人界面不展示该字段为「推荐理由」**，仅后台/BI 可见。上架后可关联 30/60 天 GMV（报表可后置） | 数据验收 |

**Owner 疑问落地结论（实现默认）**

| 疑问 | 结论 |
|------|------|
| 爬虫改每天的成本？ | MVP **不承诺日频**；继续消费雷达现网周频产物。若业务要日频，单独立项评估爬取+存储成本。 |
| 提案里标 generation_source 会影响审核吗？ | **不会**：该字段为系统元数据；审批页默认隐藏；BI/报表使用。 |
...
|------|--------|----------|
| 竞品网站 | F1 | 本节展开 |
| 社媒舆情 | F2 | [Appendix C](#appendix-c--社媒证据来源字段) |
| AZ 相似款销量 | F3 | [Appendix D](#appendix-d--内部同款匹配来源字段) |
| BOM 成本链 | F5 | [Appendix E](#appendix-e--bom-成本链来源字段) |
| 库存可生产性 | F6 | 本节 + 库存字段表 |
...
> **业务定义**：竞品站「本轮上新」的款色，按官网列表序（销量向站可称热销）优先呈现，保证 **不漏可跟的上新机会**。  
> **事实源**：竞品监控雷达 **AT 轨** Excel/GSheet（侧方案 §4.2、`上新下架表逻辑说明.md`）。
...
```text
上新机会 = 上新表 LEFT JOIN 同批原始排序表.本周排序
排序     = ORDER BY list_rank ASC NULLS LAST   （仅 sales_proxy 默认池）
主键     = normalize(product_key):::normalize(color_name)  → skc_key
```
...
```text
new  := 上新表 WHERE batch=:b AND source_line='atelier' AND site=:s
rank := 原始排序表 WHERE batch=:b AND site=:s
opp  := new LEFT JOIN rank ON skc_key
opp.list_rank = coalesce(本周排序, 排序)
opp.rank_joined = list_rank IS NOT NULL
-- URL 不参与自动 join
```
...
1. 样例 Excel：有排名行顺序 = 按本周排序升序。  
2. 无排名行标记「上新未上榜」，默认筛掉。  
3. 初始化周机会=0 + 提示。  
4. BD 伪装源整批失败、零写入。  
5. 同 ISO 周两批不串配。  
6. 方案可回溯到 snapshot_row_id。
...
```text
if 无库存数据源: producibility = unknown（展示「库存数据未接，需人工判断」）
else if low_inventory_risk: producibility = risky
else: producibility = ok
```
...
1. 接上库存只读后，相似款有库存 → 多为 ok。  
2. 主料无库存 → risky。  
3. 库存源不可用 → unknown，不阻断建稿，AI/审批高亮「需人工确认可生产性」。
...
**输出四档**：`recommend` | `cautious` | `mto` | `reject`  
（建议推 / 谨慎 / 建议 MTO / 不建议推）
...
多规则：允许集取交集；必填字段取并集。  
`risk_ack_code` **仅运营填**，LLM 不得自填。

软闸门：可 override 后交审批；`scheme_version` 变更废止旧建议。  
输出结构：结论 → 证据（引用 gates）→ 建议动作 → 何种新证据会改结论。
...
1. 列表靠前以 `list_rank` 为准；仅 sales_proxy 称「热销」。  
2. 上周/本周 = 轮次基线 diff。  
3. AT 站点与 rank_semantics 上线前标定。  
4. BOM、库存只读账号可通。  
5. VOC「商品问题」code 可映射；未就绪则仅社媒人工支路。  
6. 选款 create-from-hub 支持 AT + `system_hub_at`。  
7. 库存字段名以 ERP 联调锁定为准（见 F6）。
...
| 阶段 | 内容 | 时长感 |
|------|------|--------|
| M1 | F1 ingest + 机会池 + F2 社媒 | 2–3 周 |
| M2 | F4 草稿 + F5 成本 + F3 相似款 + F6 库存可生产性 | 3–4 周 |
| M3 | F7 AI + F8 审批同步 + F9 埋点 | 2–3 周 |
| M4 | AT 试运行、金样例、时长基线 | 2 周 |
...
1. F1：批次隔离、BD 拒绝、list/hot 分离、回溯 snapshot_row。  
2. F5：BOM valid/stale/多行、币种一致、价带 SPU≥5。  
3. F6：库存通或明确 unknown。  
4. F7：允许集违背=0；分档样例达标。  
5. F8：同事务 Sync_Pending；Abandoned 计分母。  
6. F9：generation_source 唯一；审批 UI 不展示为推荐理由。  
7. KR2 抽样基线已采集。
...
```text
peers := AT SKC 按 match_level 匹配
rep_sales := MEDIAN(sales_12m)
rep_return := SUM(return)/SUM(sold)  or unknown if SUM(sold)=0
signal := poor | good | unknown   -- 见 F3
```
...
```sql
SELECT bqvd.amount AS apply_price, bq.currency, bqv.version_no, bqv.updated_at, bq.status
FROM ecshop.bom_quotation bq
JOIN ecshop.bom_quotation_version bqv
  ON bqv.quotation_id = bq.id AND bqv.version_no = bq.current_version_no
JOIN ecshop.bom_quotation_version_detail bqvd
  ON bqvd.version_id = bqv.id AND bqvd.category = 'APPLY_PRICE'
WHERE bq.dd_goods_id = :dd_goods_id
  AND bq.status IN ('APPROVED','ACTIVE')  -- 联调锁定实枚举
  AND bqvd.amount IS NOT NULL;
-- 应用层：cardinality==1 且未过期 → valid
-- cardinality==1 且 updated_at 过旧 → stale
-- 否则 invalid；多行需 UI 点选 detail_id
```
